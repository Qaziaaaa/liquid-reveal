// src/liquid-reveal.js
var VERSION = "2.0.0";
var DEFAULT_PARAMS = {
  /* ---- assets ---- */
  base: null,
  // filled with the bundled placeholder when available
  chrome: null,
  imgWidth: 1280,
  imgHeight: 720,
  ...
  /* ======================================================================
     ...
     ====================================================================== */
  simLong: 288,
  // cells on the long axis. cells stay square, so
  // ripples are circular in screen space.
  simHz: 120,
  // fixed sim rate. accumulator-driven, so identical
  // behaviour on 60Hz and 120Hz displays.
  simMaxSteps: 4,
  // catch-up cap after a stall
  waveC: 0.5,
  // Courant number. CFL limit for this stencil is
  // 1/sqrt(2) = 0.707. Do not exceed it or it explodes.
  damping: 0.945,
  // per sim STEP. Faster decay for a cleaner, less
  // syrupy trail.
  viscosity: 0.045,
  // diffusion term. Lower keeps the ripple edge sharp.
  // Must stay under 0.25 for stability.
  edgeAbsorb: 0.9,
  // multiplier at the very border (absorbing layer,
  // stops waves bouncing off the frame)
  edgeCells: 16,
  // width of that absorbing layer, in cells
  hClamp: 1.1,
...
var isEl = (n) => n && typeof n === "object" && (typeof Element !== "undefined" && n instanceof Element || typeof n.nodeType === "number");
...
var PLACEHOLDER_BASE = placeAt("../assets/base-placeholder.png");
var PLACEHOLDER_CHROME = placeAt("../assets/chrome-placeholder.png");
var VERT = `
...
var LiquidReveal = class {
  ...
  constructor(target, options = {}) {
    ...
    if (options) for (const k in options) {
      if (options[k] !== void 0) this.params[k] = options[k];
    }
    this.params.base = options.base ?? PLACEHOLDER_BASE;
    this.params.chrome = options.chrome ?? PLACEHOLDER_CHROME;
    this.debug = !!options.debug;
    this.urlParams = !!options.urlParams;
    this.exportButton = options.exportButton !== true && options.exportButton ? options.exportButton : options.exportButton === true;
  ...
  onReady(fn) {
    ...
    if (this._ready) fn(this);
    return this;
  }
  onError(fn) {
    this._errorCbs.push(fn);
    if (this._felled) fn(this);
    return this;
  }
  /* ==========================================================================
     DOM / wiring
    ...
    if (expTarget) {
      const hov = (e) => {
        if (e.type === "mouseenter") {
          expTarget.style.borderColor = "#C98247";
          expTarget.style.color = "#C98247";
        } else {
          expTarget.style.borderColor = "rgba(255,255,255,.18)";
          expTarget.style.color = "rgba(255,255,255,.75)";
        }
      };
      const act = () => {
        expTarget.style.transform = "scale(.94)";
        setTimeout(() => {
          expTarget.style.transform = "";
        }, 120);
      };
      this._on(expTarget, "mouseenter", hov);
      this._on(expTarget, "mouseleave", hov);
      this._on(expTarget, "click", () => {
        act();
        this.exportFrame();
      });
  ...
  _init() {
    ...
    if (this.isGL2) {
      if (gl.getExtension("EXT_color_buffer_float") || gl.getExtension("EXT_color_buffer_half_float")) {
        simType = gl.HALF_FLOAT;
        simInternal = gl.RGBA16F;
        simMode = "rgba16f";
        PACKED = false;
      }
    } else {
    ...
    const buildSim = (w, h) => {
      ...
      disposeTarget(this.simA);
      disposeTarget(this.simB);
      disposeTarget(this.viewRT);
      this.SW = nw;
      this.SH = nh;
      ...
      this._read = this.simA;
      this._write = this.simB;
      clearSim();
      return true;
    };
    const noiseRT = makeTarget(
      this.params.noiseSize,
      this.params.noiseSize,
      gl.UNSIGNED_BYTE,
      gl.RGBA,
      gl.LINEAR
    ...
    if (!noiseRT) {
      this._bail("render target creation failed");
      return false;
    }
    ...
    this.progSim = progSim;
    this.progComp = progComp;
    this.progView = progView;
    this.progNoise = progNoise;
    this.uSim = uSim;
    this.uComp = uComp;
    this.uView = uView;
    this.uNoise = uNoise;
  ...
  _resize() {
    ...
    if (ok === null) {
      this._bail("sim target creation failed");
      return false;
    }
    return true;
  }
  ...
  _stepSim(now, dt) {
    ...
    let idling = false;
    const idleScale = this.W < this.H ? P.narrowScale : 1;
    if (P.idleDrift && !this.REDUCED && (!anyActive || idleFor > P.idleDelayMs)) {
      idling = true;
      const tt = (now - this.t0) / 1e3 * P.idleSpeed;
      ptr.x = P.idleCenter[0] + Math.cos(tt * 1) * P.idleRadius[0] + Math.cos(tt * 2.3) * P.idleRadius[0] * 0.28;
    ...
    if (idling) {
      ptr.px = ptr.x;
      ptr.py = ptr.y;
    } else for (const p of this.pointers.values()) {
      p.px = p.x;
      p.py = p.y;
    }
    return true;
  ...
  _stepNoise(now) {
    ...
    gl.uniform1f(this.uNoise.u_time, (now - this.t0) / 1e3);
  ...
  _frame(now) {
    ...
    let dt = (now - this.prevT) / 1e3;
    const delta = now - this.prevT;
    this.prevT = now;
    if (dt > 0.05) dt = 0.05;
    if (dt <= 0) return;
    const quiet = !this.params.idleDrift && this.energy_ < this.params.idleStopEps && now - this.ptr.lastInput > this.params.idleDelayMs;
    ...
    if (now - this.lastFpsAt > 500) {
      this.fps_ = Math.round(this.fpsAcc * 1e3 / (now - this.lastFpsAt));
      this.fpsAcc = 0;
      this.lastFpsAt = now;
  ...
  _staticFrame() {
    this._resize();
    this._clearSim();
    this._simStep(0.42, 0.6, 0.42, 0.6, 0.26, false);
    for (let i = 0; i < 26; i++) this._simStep(0, 0, 0, 0, 0, false);
    this._simStep(0.6, 0.5, 0.6, 0.5, 0.24, false);
    for (let i = 0; i < 22; i++) this._simStep(0, 0, 0, 0, 0, false);
    this._simStep(0.5, 0.63, 0.5, 0.63, 0.2, false);
  ...
  _boot() {
    ...
    this.acc = 0;
    this.SIM_DT = 1 / this.params.simHz;
    this.energy_ = 0;
    this.frameMs_ = 0;
    this.fps_ = 0;
    this.fpsAcc = 0;
  ...
  setImages({ base, chrome } = {}) {
    if (base !== void 0) this.params.base = base;
    if (chrome !== void 0) this.params.chrome = chrome;
    if (!this.running || this.disposed) return this;
    const jobs = [];
    if (base !== void 0) jobs.push(this._loadImage(this.params.base, this.texBase));
    if (chrome !== void 0) jobs.push(this._loadImage(this.params.chrome, this.texChrome));
  ...
  impulse(x, y, amp) {
    this.pending = { x, y, amp: amp === void 0 ? 0.5 : amp };
    this.needsDraw = true;
    return this;
  }
...
var liquid_reveal_default = LiquidReveal;
export {
  DEFAULT_PARAMS,
  LiquidReveal,
  VERSION,
  liquid_reveal_default as default
};
